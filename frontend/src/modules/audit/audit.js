...
document.getElementById("transfer").onclick = async () => {
  // TODO(member4): await wc.audit.transferHolder(recordId(val("key")), val("newOwner"))
  out("TODO(member4): implement transferHolder call");
};
document.getElementById("revoke").onclick = async () => {
  // TODO(member4): await wc.audit.revokeCredential(recordId(val("key")), val("reason"))
  out("TODO(member4): implement revoke call");
};
document.getElementById("checkpoint").onclick = async () => {
  // TODO(member4): await wc.audit.logCredentialEvent(recordId(val("key")), val("note"))
  out("TODO(member4): implement logCredentialEvent call");
};
