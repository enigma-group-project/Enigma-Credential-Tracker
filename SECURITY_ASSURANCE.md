# Security Assurance — Enigma-Credential-Tracker
...
| **[M] Mechanical** | Reentrancy protection on state-changers; access control enforced; CEI ordering; no unchecked external-call return; events on state change. | ✅ OZ `ReentrancyGuard` on `issueCredential` + AuditTrail mutators; OZ `Ownable` (Registrar) + Institution role; CEI; events on every mutation. |
...
- **Design & implement:** chose a registry/attestation model (not a token); built from reviewed OZ components (`Ownable`, `ReentrancyGuard`, `Pausable`); defined roles (Registrar, Institution, Owner), state (`Status`), and transfer/revoke/renew rules.
- **Harden & verify:** enforce access control (themed `onlyRegistrar`/`onlyTestCenter`/`onlyController`); preserve state invariants; protect external interactions (CEI + guard); test edge cases + fuzz + invariants.
...
|---|---|---|
| Reentrancy (SC05) | `issueCredential`, AuditTrail mutators | OZ `ReentrancyGuard` + CEI (effects before any external call) |
| Broken access control (SC01) | issue / setController / revoke / pause | `onlyTestCenter` / `onlyRegistrar` / `onlyController`; OZ `Ownable` for Registrar |
| Unauthorized state change | `controller*` mutators | `onlyController`; controller is set once (`ControllerSet`) by the Registrar |
| Fake issuer | `issueCredential` | gated by `IssuerRegistry.isInstitution` (Registrar-granted) |
| Replay / duplicate record | `issueCredential` | `AlreadyExists` check on the record id |
| Denial via stuck state | issuance | `Pausable` lets the Registrar halt issuance during an incident, then `unpause` |
| Front-running | `revokeCredential` | issuer/Registrar-only; gas price does not change correctness |
